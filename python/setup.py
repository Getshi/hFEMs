...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        #                 cmake_args, cwd=self.build_temp, env=env, stdout=PIPE, stderr=PIPE)
        # subprocess.run(['cmake', '--build', '.'] +
        #                       build_args, cwd=self.build_temp, stdout=PIPE, stderr=PIPE)
# to avoid compiling with wrong pip on cluster
...
setup(
    ...
    ext_modules=[ # extension modules
        CMakeExtension('hFEMs._solver')],
    cmdclass=dict(build_ext=CMakeBuild),
    zip_safe=False,
    packages=[],  # extra python packages to include # 'HYLC.AnotherModule',
)
